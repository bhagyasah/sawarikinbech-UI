...
class EnquiryList extends React.Component {
  ...
  sendEmail = (id) => {
    ...
    const emailToSend = main.initialAdminData.serviceCenterEnquiry.find(vb => vb.id === id);
  ...
  render() {
    ...
    return (
      <div className="admin-add-brand-content-table">
        <div style={{ width: '100%' }}>
          {main.initialAdminData.serviceCenterEnquiry && (
            <Table
              ...
              data={main.initialAdminData.serviceCenterEnquiry.filter(vb => !vb.markStatus).map(obj => (
                {
                  ...obj,

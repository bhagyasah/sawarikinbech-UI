...
class EnquiryList extends React.Component {
  ...
  sendEmail = (id) => {
    ...
    const emailToSend = main.initialAdminData.dealerEnquiry.find(vb => vb.id === id);
  ...
  render() {
    ...
    return (
      <div className="admin-add-brand-content-table">
        <div style={{ width: '100%' }}>
          {main.initialAdminData.dealerEnquiry && (
            <Table
              ...
              data={main.initialAdminData.dealerEnquiry.filter(vb => !vb.markStatus).map(obj => (
                {
                  ...obj,

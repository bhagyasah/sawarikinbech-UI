...
class BrandList extends React.Component {
  ...
  updateBrand = (id) => {
    ...
    const brandToUpdate = main.initialAdminData.serviceCenterList.find(vb => vb.id === id);
    updateFormValue('addServiceCenter', { ...main.initialAdminData.serviceCenterList, ...brandToUpdate });
  ...
  mainDelete = async () => {
    const { deleteServiceCenter } = this.props;
    const { showDelete } = this.state;
    this.setState({ loading: true, success: null, error: null });
    const res = await deleteServiceCenter(showDelete);
    if (res) {
  ...
  render() {
    ...
    return (
      <div className="admin-add-brand-content-table">
        <div style={{ width: '100%' }}>
          {main.initialAdminData.serviceCenterList && (
            <Table
              headers={{ id: 'Id', city: 'City', name: 'Name', description: 'Description', image: 'Image', update: 'Update', delete: 'Delete' }}
              data={main.initialAdminData.serviceCenterList.filter(vp => (parseInt(vp.stypeId, 10) === parseInt(form.addServiceCenter.stypeId, 10) && parseInt(vp.sbId, 10) === parseInt(form.addServiceCenter.sbId, 10))).map(obj => (
                {
                  ...obj,
        ...
        <AddServiceCenterForm isOpen={showUpdate} onClose={this.toggleAddBrandForm} props={this.props} update />
      </div>
    );
  }
}

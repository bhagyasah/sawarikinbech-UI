...
const mapBrand = (props) => {
  ...
  const res = main.initialAdminData.vehicleBrand.filter(vb => vb.stypeId === parseInt(form.addServiceCenter.stypeId, 10)).map(vb => ({ label: vb.brandName, value: vb.id }));
...
export default (props) => {
  ...
  <form style={{ width: 'auto', display: 'flex', flexDirection: 'row'}}>
    <Form contents={addBrandStructure(props)} {...props} schema="addServiceCenter" />
  </form>
  );
};
